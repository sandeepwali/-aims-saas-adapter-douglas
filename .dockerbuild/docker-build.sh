...
if [ -z "${CI_REGISTRY_USER}" ]; then {
  echo >&2 "Please provide \$CI_REGISTRY_USER 'registry-ci-usern' variable before running this script."
  exit 1
}; fi
if [ -z "${CI_REGISTRY_PASSWORD}" ]; then {
  echo >&2 "Please provide \$CI_REGISTRY_PASSWORD 'registry-ci-password' variable before running this script."
  exit 1
}; fi

if [ -z "${APP_NAME}" ]; then {
  echo >&2 "Please provide \$APP_NAME variable before running this script."
  exit 1
}; fi
if [ -z "${APP_REGISTRY_IMAGE}" ]; then {
  echo >&2 "Please provide \$APP_REGISTRY_IMAGE variable before running this script."
  exit 1
}; fi
if [ -z "${APP_MAINTAINER}" ]; then {
  echo >&2 "Please provide \$APP_MAINTAINER variable before running this script."
  exit 1
}; fi
if [ -z "${APP_COMPONENT}" ]; then {
  echo >&2 "Please provide \$APP_COMPONENT variable before running this script."
  exit 1
}; fi
if [ -z "${APP_PART_OF}" ]; then {
  echo >&2 "Please provide \$APP_PART_OF variable before running this script."
  exit 1
}; fi
